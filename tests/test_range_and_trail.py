...
from trajectory_simulation.flightDataCalculation import (
    calculate_descending_angle, get_trajectory_metrics)
...
def test_range_properties():
    ...
    rs._apex = 5.0
    assert (
        pytest.approx(rs.distance_yards, rel=1e-5)
        == np.linalg.norm([10.0, 0.0]) * 1.09361
    )
