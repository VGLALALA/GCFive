...
def run_hough_with_radius(image):
    ...
    if circles is not None:
        ...
        h, w = image.shape[:2]
        best = sorted(
            circles, key=lambda c: (c[0] - w // 2) ** 2 + (c[1] - h // 2) ** 2
        )[:2]
        for idx, (x, y, r) in enumerate(circles):
            ...
            overlap_score = calculate_overlap_score(
                (x, y, r), [c for i, c in enumerate(circles) if i != idx]
            )
            clarity_score = calculate_clarity_score(cropped_img)
            total_score = (overlap_score + clarity_score) / 2
            print(
                f"Circle {idx + 1} Score: {total_score:.2f} (Overlap: {overlap_score:.2f}, Clarity: {clarity_score:.2f})"
            )
